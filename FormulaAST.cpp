    ...
    namespace {
        ...
        public:
            ...
            double Evaluate(const SheetInterface& sheet) const override {
                ...
                CellInterface::Value value = sheet.GetCell(*cell_) ? sheet.GetCell(*cell_)->GetValue() : 0.0;
                ...
                if (std::holds_alternative<std::string>(value)) {
                    try {
                        return TransformStringToDoble(std::get<std::string>(value));
                    }
                    catch (...) {
        ...
        private:
            ...
            static double TransformStringToDoble(const std::string& str) {
                if (str.empty()) {
                    return 0.0;
                }

                size_t pos;
                double d_value = std::stod(str, &pos);
                if (pos == str.length()) {
                    return d_value;
                }
